...
def load_and_clean_csv(uploaded_file):
    ...
    Cary exports may report time in either seconds or minutes. Time in minutes
    is converted to seconds immediately so every downstream calculation uses
    seconds and V₀ remains in µM/s.
    """
    ...
    for i, line in enumerate(lines):
        header = line.lower()
        if "time (sec)" in header or "time (s)" in header:
            start_idx = i
            time_multiplier = 1.0
            break
        if "time (min)" in header or "time (mins)" in header:
            start_idx = i
    ...
    if start_idx is None:
        raise ValueError(
            "Could not find a time header. The app recognizes "
            "'Time (sec)' and 'Time (min)'."
        )
    ...
    df["Time"] = df["Time"] * time_multiplier
...
# Preserve review settings separately from Streamlit widget keys. Streamlit
# deletes widget keys when Stage 2 hides the time-course controls.
...
def review_widget_keys(filename):
    run_id = safe_key(filename)
    return tuple(f"{prefix}{run_id}" for prefix in REVIEW_WIDGET_PREFIXES)


def save_review_settings(runs):
    """Save every current review choice before leaving the time-course page."""
    st.session_state["saved_review_settings"] = {
        run["filename"]: {
            key: st.session_state[key]
            for key in review_widget_keys(run["filename"])
...
def restore_review_settings(filename):
    """Restore saved values before the review widgets are constructed."""
    saved = st.session_state.get("saved_review_settings", {}).get(filename, {})
    for key in review_widget_keys(filename):
...
if st.session_state.get("upload_signature") != upload_signature:
    ...
    st.session_state.pop("saved_review_settings", None)
...
def render_run_review(
    ...
    df = run["df"]
    restore_review_settings(filename)
...
if st.session_state["workflow_stage"] == "review":
    ...
    if st.button(
        ...
        if not snapshot:
            ...
            save_review_settings(loaded_runs)
...
st.caption(
    "Uncheck Include to remove a point from the plot and kinetic-model fit; "
    "check it again to restore it. The plot and parameters update automatically, "
    "and your choice is also saved when you return to the individual time courses."
)
...
# Only the Include checkbox is editable; the V₀ and interval values remain
# controlled from the time-course review page. Keep row order fixed so each
# checkbox maps unequivocally to the corresponding filename.
edited_summary = st.data_editor(
    summary_display,
    hide_index=True,
    use_container_width=True,
    num_rows="fixed",
    disabled=[col for col in summary_display.columns if col != "Include"],
    column_config={
        "Include": st.column_config.CheckboxColumn(
            "Include", help="Use this run in the kinetic fit and plot.", required=True
        ),
    },
    key="kinetic_inclusion_editor",
)

# The table returns the currently edited values on each rerun. Persist them in
# both the Stage 2 snapshot and the Stage 1 backing store, otherwise returning
# to the time-course page would silently restore old inclusion choices.
for row_number, edited_include in enumerate(edited_summary["Include"].tolist()):
    included = bool(edited_include)
    result = snapshot[row_number]
    result["Include"] = included
    filename = result["File"]
    checkbox_key = f"include_{safe_key(filename)}"
    st.session_state.setdefault("saved_review_settings", {}).setdefault(
        filename, {}
    )[checkbox_key] = included
    # Do not set the Stage 1 widget key here: Streamlit owns it while that
    # checkbox exists. restore_review_settings() restores the backing value
    # when Stage 1 is shown again.
...
if len(included_df) < 3:
    st.warning(
        ...
        "Check additional runs in the table above."
    )
    st.stop()
...
# Build parameter rows first, so their formatted estimates can appear above
# the plot. curve_fit's pcov provides approximate unweighted-fit standard
# errors; these are not independent experimental replicate uncertainties.
s_max = max(float(np.max(S)) * 1.08, 1.0)
s_plot = np.linspace(0.0, s_max, 400)
parameter_rows = []

def parameter_standard_errors(covariance, n_parameters):
    """Return usable SEs or NaNs when a fit's covariance is unavailable."""
...
if "MM" in fit_outputs:
    ...
    vmax, km = popt
    errors = parameter_standard_errors(pcov, 2)
...
if "INH" in fit_outputs:
    ...
    vmax, km, ki = popt
    errors = parameter_standard_errors(pcov, 3)
    parameter_rows.append(
        {
...
def estimate_with_error(value, se, unit):
    """Format the fit estimate and its standard error without false precision."""
    if not np.isfinite(value):
        return "Unavailable"
    if not np.isfinite(se):
        return f"{value:.3g} {unit} (SE unavailable)"
    return f"{value:.3g} ± {se:.2g} {unit}"


st.subheader("Fitted parameters")
for row in parameter_rows:
    if model_choice == "Compare both models":
        st.markdown(f"#### {row['Model']}")
    kcat_col, km_col, ki_col = st.columns(3)
    if enzyme_nM > 0:
        enzyme_uM = enzyme_nM / 1000.0
        kcat_col.metric(
            "kcat",
            estimate_with_error(
                row["Vmax (µM/s)"] / enzyme_uM,
                row["Vmax SE"] / enzyme_uM,
                "s⁻¹",
            ),
        )
    else:
        kcat_col.metric("kcat", "Not calculated")
    km_col.metric(
        "Km",
        estimate_with_error(row["Km (mM)"], row["Km SE"], "mM"),
    )
    if np.isfinite(row["Ki (mM)"]):
        ki_col.metric(
            "Ki",
            estimate_with_error(row["Ki (mM)"], row["Ki SE"], "mM"),
        )
    else:
        ki_col.metric("Ki", "Not in MM model")

st.caption(
    "± values are approximate standard errors from the unweighted nonlinear "
    "fit, not variation across independent experiments. kcat SE assumes the "
    "entered enzyme concentration is exact."
)
if enzyme_nM <= 0:
    st.info("Enter a positive final enzyme concentration in Assay settings to calculate kcat.")

# Plotly supports hover coordinates and zoom; plotting only included runs means
# unchecking a point immediately removes it from both the graph and the fit.
fig_mm = go.Figure()
fig_mm.add_trace(
    go.Scatter(
        x=S,
        y=v,
        mode="markers",
        name="Included experimental V₀",
        marker={"size": 10},
        customdata=included_df["File"].to_numpy(),
        hovertemplate=(
            "[pyruvate] = %{x:.4g} mM<br>"
            "V₀ = %{y:.5g} µM/s<br>"
...
if "MM" in fit_outputs:
    ...
    fig_mm.add_trace(
        go.Scatter(
            x=s_plot,
            y=michaelis_menten(s_plot, *popt),
            mode="lines",
            name="Michaelis–Menten",
            hovertemplate=(
                "[pyruvate] = %{x:.4g} mM<br>"
                "Modeled V₀ = %{y:.5g} µM/s<extra></extra>"
            ),
        )
    )
...
if "INH" in fit_outputs:
    ...
    fig_mm.add_trace(
        go.Scatter(
            x=s_plot,
            y=substrate_inhibition(s_plot, *popt),
            mode="lines",
...
fig_mm.update_layout(
    height=510,
    xaxis_title="[pyruvate], mM",
    yaxis_title="Initial velocity V₀ (µM/s)",
    hovermode="closest",
    legend={"orientation": "h", "y": -0.2},
    margin={"l": 45, "r": 20, "t": 25, "b": 65},
)
fig_mm.update_xaxes(showgrid=True, gridcolor="rgba(140,140,140,0.17)")
fig_mm.update_yaxes(showgrid=True, gridcolor="rgba(140,140,140,0.17)")
st.plotly_chart(fig_mm, use_container_width=True, key="interactive_kinetic_plot")
st.caption("Hover over a point for its exact coordinates and filename. The graph can also be zoomed.")
