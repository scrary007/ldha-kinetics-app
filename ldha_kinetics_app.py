...
for run in loaded_runs:
    ...
    with st.expander(f"{filename}", expanded=True):
        ...
        # Re-read in case a widget changed on this rerun.
        pyruvate_mM = float(st.session_state[pyr_key])
        ...
        if reset_col.button("Reset to suggested interval", key=f"reset_{key}"):
            st.session_state[interval_key] = (suggestion["start_s"], suggestion["end_s"])
            st.session_state[start_key] = suggestion["start_s"]
            st.session_state[end_key] = suggestion["end_s"]
            st.rerun()

        # Recalculate after any interval edits.
        selected_start, selected_end = st.session_state[interval_key]
        diagnostics = regression_diagnostics(
            df,
            selected_start,
