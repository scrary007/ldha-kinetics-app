...
# =============================================================================
def mark_kinetic_model_stale():
    """Hide model fitting after an accepted run value has been changed."""
    st.session_state["kinetic_model_ready"] = False
...
def render_run_review(
    ...
    with st.expander(f"{filename}", expanded=True):
        ...
        meta1.checkbox(
            ...
            key=include_key,
            on_change=mark_kinetic_model_stale,
        )
        meta2.number_input(
            "Final pyruvate [S] (mM)",
            min_value=0.0,
        ...
        if diagnostics["half_diff_pct"] > half_slope_diff_limit:
            ...
            diagnostics["thirds_spread_pct"] > AUTO_THIRDS_SPREAD_LIMIT_PCT
            or diagnostics["quadratic_slope_change_pct"]
                > AUTO_QUADRATIC_SLOPE_CHANGE_LIMIT_PCT
        ):
            st.warning(
                "The local slope changes systematically across this interval, suggesting curvature. "
                "Consider shortening or shifting the interval before accepting V₀."
            )
        else:
            st.success(
        ...
        st.session_state[result_key] = {
            ...
            "Quadratic_slope_change_pct": diagnostics["quadratic_slope_change_pct"],
        }
...
def render_summary_and_models(result_keys, enzyme_nM):
    ...
    st.button(
        ...
        help="Use this after changing one or more run intervals. It refreshes the V₀ table without redrawing every time-course plot.",
    )
    st.caption(
        "The individual run controls above update independently for speed. "
        "Click this button after making changes to refresh the summary table."
    )
    ...
    if "kinetic_model_ready" not in st.session_state:
        st.session_state["kinetic_model_ready"] = False

    st.divider()
    if not st.session_state["kinetic_model_ready"]:
        st.markdown(
            "When you are satisfied with the individual V₀ intervals and inclusion choices, "
            "continue to kinetic-model fitting."
        )
        if st.button(
            "Use these V₀ values for kinetic-model fitting",
            type="primary",
            use_container_width=True,
        ):
            st.session_state["kinetic_model_ready"] = True
